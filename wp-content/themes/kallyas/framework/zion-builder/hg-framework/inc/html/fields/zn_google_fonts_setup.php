...
class ZnHgFw_Html_Zn_Google_Fonts_Setup extends ZnHgFw_BaseFieldType {
	...
	/**
	 * Holds a reference to all Google Fonts
	 *
	...
	public function render_repeater_content( $schema, $model = array() ) {
		...
		foreach ($this->all_fonts as $font_data) {
			if ($font_data['family'] == $selected_font) {
				$fonts_data = $font_data;
				break;
			}
		}

		if ($fonts_data && isset($fonts_data['variants']) && is_array($fonts_data['variants']) ) {
			foreach ($fonts_data['variants'] as $font_variant) {
				$variants[$font_variant] = $font_variant;
			}
		}
