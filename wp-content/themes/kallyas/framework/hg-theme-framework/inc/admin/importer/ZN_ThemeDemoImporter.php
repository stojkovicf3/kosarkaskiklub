...
class ZN_ThemeDemoImporter {
	 ...
	 * @return int|string
	 */
	public static function getDemoDirPath( $demo_id = '' ) {
		if ( empty( $demo_id ) ) {
			if ( ! isset( $_POST['demo_name'] ) || empty( $_POST['demo_name'] ) ) {
				return 0;
			}
			$demo_id = esc_attr( wp_strip_all_tags( $_POST['demo_name'] ) );
		}
		return self::getTempFolder() . '/' . $demo_id;
	}
	...
	public function ajaxInstallDemo() {
		...
		add_filter( 'wp_insert_term_data', array( __CLASS__, 'tryToKeepTermIdOnImport' ), 10, 3 );
		...
		$demo_id     = ZN_DemoImportHelper::sanitizeString( $_POST['demo_name'] );
		$step        = ZN_DemoImportHelper::sanitizeString( $_POST['step'] );
		$demoDirPath = self::getDemoDirPath( $demo_id );
		...
		if ( self::STEP_DO_GET_DEMO == $step ) {
			...
			// else, get the demo
				...
				$demoDirPath = self::__getDemo( $demo_id );
	 ...
	 * @return bool|string
	 */
	private static function __getDemo( $demo_id ) {
		global $wp_filesystem;
		...
		//#!
		$download_url = kallyas()->server_api->get_demo_download_url( $demo_id );

		if ( is_wp_error( $download_url ) ) {
			return false;
		}

		$temp_file_location = download_url( $download_url );

		if ( is_wp_error( $temp_file_location ) ) {
			return false;
		}

		$saveDirPath = self::getTempFolder() . '/' . $demo_id . '/';
		if ( ! $wp_filesystem->is_dir( $saveDirPath ) ) {
			wp_mkdir_p( $saveDirPath );
		}

		$unzipped = unzip_file( $temp_file_location, $saveDirPath );

		if ( is_wp_error( $unzipped ) ) {
			// delete the temp file
			wp_delete_file( $temp_file_location );
		}
	 ...
	 */
	private static function __respond( $data = array(
		'state' => '',
		'msg'   => '',
	) ) {
		remove_filter( 'wp_insert_term_data', array( __CLASS__, 'tryToKeepTermIdOnImport' ), 10, 3 );
		delete_transient( self::DEMO_INSTALLING_TRANSIENT );
